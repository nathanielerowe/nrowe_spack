...
class Wirecell(Package):
    ...
    depends_on('boost cxxstd=17 +'+boost_variants)

    # We need one or the other.
    depends_on('jsonnet +python', when='+cppjsonnet')
    depends_on('go-jsonnet +python', when='~cppjsonnet')
    ...
    depends_on('root ~opengl cxxstd=17', when='+root')
    ...
    def install(self, spec, prefix):
        ...
        #     "--with-cuda=no"
        # ]
        cfg += ["--with-cuda=/opt/nvidia/hpc_sdk/Linux_x86_64/23.9/cuda/12.2", 
                "--with-cuda-lib=/opt/nvidia/hpc_sdk/Linux_x86_64/23.9/cuda/12.2/lib64",
                "--with-cuda-include=/opt/nvidia/hpc_sdk/Linux_x86_64/23.9/cuda/12.2/include"]# added cuda stuff

        cfg += ["--with-libtorch=" + spec['py-torch'].prefix.lib+"/torch/python3.10/site-packages/torch", 
                "--with-libtorch-lib=" + spec['py-torch'].prefix.lib+"/python3.10/site-packages/torch/lib",
                "--with-libtorch-libs=torch, c10, torch_cuda, c10_cuda, torch_cpu",
                "--with-libtorch-include=" + spec['py-torch'].prefix.lib+"/python3.10/site-packages/torch/include"]# added torch stuff
